...
size_t Manifold_no_vertices(HMesh::Manifold* self) {
	return self->no_vertices();
}

size_t Manifold_no_faces(HMesh::Manifold* self) {
	return self->no_faces();
}

size_t Manifold_no_halfedges(HMesh::Manifold* self) {
	return self->no_halfedges();
}

// for each polygon is defined as a polycount + a index per vertex. E.g. a single triangle takes up 4 integers
size_t Manifold_no_polygon_indices(HMesh::Manifold* self) {
	size_t res = 0;
	for (auto f : self->faces()) {
		res++;
		circulate_face_ccw(*self, f, [&](VertexID v) {
			res++;
		});
	}
	return res;
...
void vertex_normal(const Manifold* m_ptr, size_t _v, double* norm) {
	CGLA::Vec3d n = normal(*m_ptr, VertexID(_v));
	norm[0] = n[0];
	norm[1] = n[1];
	norm[2] = n[2];
}
...
void face_normal(const Manifold* m_ptr, size_t _f, double* norm) {
	CGLA::Vec3d n;
